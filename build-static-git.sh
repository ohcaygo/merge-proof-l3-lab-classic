...
tar -xf git.tar.xz
cd git-2.50.1
# Rename only libidn2's private gnulib diagnostic symbol inside the disposable
# static archive, preserving all of its internal references. Git's distinct
# error() function has a different signature and must never be interposed.
sha256sum /usr/lib/libidn2.a > "$TASK_OUTPUT/libidn2-before.txt"
objcopy --redefine-sym error=merge_proof_idn2_error /usr/lib/libidn2.a
sha256sum /usr/lib/libidn2.a > "$TASK_OUTPUT/libidn2-after.txt"
